...
Usage:
  ...
  *** mkdocs.py must be run from the dataformats directory.
'''
curdir = os.path.basename(os.environ["PWD"])
if curdir != "dataformats":
	usage()
...
CLASSMAPFILE = '../python/classmap.py'	
DEBUG=0
...
def main():
	...
	if len(args) > 0:
		...
		elif os.path.exists(PACKAGELIST):
			packages = map(strip,open(PACKAGELIST))
			filelist = []
			for package in packages:
				file =  "%sDataFormats/%s/interface/*.h" % \
				       (BASE, package)
				cmd = 'find %s' % file
				print "\t%s" % package
	...
	for index, file in enumerate(filelist):
		...
		# Let's not bother with some headers
		
		if find(file, 'DataFormats/Common') > -1: continue
		if find(file, 'DataFormats/Math') > -1: continue
		if find(file, 'classes.h') > -1: continue
