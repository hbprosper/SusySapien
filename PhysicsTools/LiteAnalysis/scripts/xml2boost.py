...
def classRecord(context, element):
    ...
    # nonconstructable.
    #--------------------------------------------------
    nondestroyable = find(context['classtype'],'dtor') > -1
    if nondestroyable: nonconstructable = \
       nonconstructable or find(context['classtype'],'ctor') > -1
...
def main():
    ...
    CONTEXT['preamble'].append("// Created: %(date)s, xml2boost.py"
                               " %(version)s" % \
                               CONTEXT)                          
    CONTEXT['preamble'].append("//$Revision: 1.31 $")
    CONTEXT['preamble'].append("//"+MAXWIDTH*"-")
    CONTEXT['preamble'].append('#include "boostpython.hpp"')
