...
Method::Method(std::string classname, 
...
Method::invoke(void* address)
{
  ...
  if ( debug_ > 0 )
    cout << "Method::invoke - execute: " << methodrecord_ << endl;
  ...
  if ( simplemethod_ )
    {
      if ( debug_ > 0 )
        cout << "Method::invoke - simple: " << methodrecord_ << endl;
  ...
  else
    {
      ...
      if ( smartpointer_ )
        {
          if ( debug_ > 0 )
            cout << "Method::invoke - smart pointer " << endl;
...
std::ostream&
Method::operator<<(std::ostream& os)
{
  os << str() << endl;
  return os;
}
