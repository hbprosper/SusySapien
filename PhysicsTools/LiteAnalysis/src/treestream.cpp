...
itreestream::_getbranches(TBranch* branch, int depth)
{
  ...
  */
  ...
  // Check for current branch for branches
...
itreestream::_getleaf(TBranch* branch, TLeaf* leaf)
{
  ...
  v.isvector= false;  // Set by caller
  v.branch  = branch;
  v.leaf    = leaf;
  ...
  if ( DEBUGLEVEL > 0 ) 
    cout << v.branchname << "\t\t" << v.leafname << endl;
}
...
itreestream::read(int entry)
{
  ...
  return localentry; // Return ordinal value within tree.
}
...
itreestream::print(ostream& out)
{
  ...
  for (int i=0; i < (int)branchname.size(); i++)
    {
      if ( data.find(branchname[i]) == data.end() ) continue;
      count++;
      
      Field& field = data[branchname[i]];
...
//         branch in the root-tuple. However, a branch is activated only if the
//         _select method has been called for that branch and, of course, 
//         if the named branch (that is, variable) exists.
//
                     ...
                     bool isvector)
{
  _statuscode = kSUCCESS;
  if ( maxsize < 1 )
    fatal("_select - external buffer for " + namen + " is of zero length!");
