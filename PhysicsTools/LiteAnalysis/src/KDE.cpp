...
//       cout << i << "\tiflag=" << iflag << "\t" << xval[i] << endl;
    ...
    for(int i=0; i < npar; i++) bandwidth[i] = xval[i];
    ...
    cout << "\tiflag=" << iflag << "\tfval=" << fval << endl << endl;
...
// This methods transform KDE objects into function
// objects. A function object is an object that can
...
{
  double nlogp = 0; 
  for(int i=0; i < _D; i++) nlogp += _bandwidth[i] * _bandwidth[i];
  nlogp =-nlogp;
  return _lambda * nlogp;
}
